...
class KarnoMap {
    constructor() {
        ...
        this.isLocked = false; // Gruplar bulunduktan sonra değişiklikleri engelle
    ...
    generateKarno2Var(container) {
        console.log('2 değişkenli Karno haritası oluşturuluyor - Grid tasarımı');
        
        const isMobile = window.innerWidth <= 768;
        const isVerySmall = window.innerWidth <= 480;
        
        // CSS ile senkronize cellSize hesabı
        ...
        // 4x4 grid yapısı: boş köşe + B ana + B değerler + A ana + A değerler + 4 minterm
        let html = `
        ...
        // Grid-based event listener'ları ekle
        this.addGridCellEventListeners();
    }

    generateKarno3Var(container) {
        console.log('3 değişkenli Karno haritası oluşturuluyor - Grid tasarımı');
        ...
        // 5x4 grid yapısı: boş köşe + BC ana + BC değerler + A ana + A değerler + 8 minterm
        let html = `
    ...
    generateKarno4Var(container) {
        console.log('4 değişkenli Karno haritası oluşturuluyor - Grid kesişim tasarımı');
        ...
        // 6x6 grid yapısı: boş köşe + CD ana + CD değerler + AB ana + AB değerler + 16 minterm
        let html = `
        ...
        // Event listener'ları JavaScript ile ekle - güncellenmiş selector
        this.addGridCellEventListeners();
    }

    // Grid-based 4 değişkenli harita için event listener'lar
    addGridCellEventListeners() {
        console.log('addGridCellEventListeners çağrıldı');
        const cells = document.querySelectorAll('.karno-cell-grid');
        console.log('Bulunan grid hücre sayısı:', cells.length);
        
        cells.forEach(cell => {
            const minterm = parseInt(cell.dataset.minterm);
            console.log('Grid event listener ekleniyor, minterm:', minterm);
            
            // Click event
            cell.addEventListener('click', (e) => {
                e.preventDefault();
                console.log('Grid hücre tıklandı, minterm:', minterm);
    ...
    toggleKarnoCell(minterm) {
        ...
        // Kilitle durumunu kontrol et
        if (this.isLocked) {
            console.log('⚠️ Harita kilitli - Grupları temizleyin veya yeni başlatın');
    ...
    renderKarnoMap() {
        ...
        // Hem eski hem yeni grid hücrelerini kontrol et
        ...
        console.log('Grid hücreleri:', gridCells.length, 'Eski hücreler:', oldCells.length);
        console.log('KarnoValues durumu:', this.karnoValues);
        ...
        cells.forEach(cell => {
            ...
            // Kilitli durumda sadece class ekle, CSS cursor'u halleder
            if (this.isLocked) {
    ...
    removeCellGroupHighlight() {
        ...
        // Tüm hover highlight'larını kesinlikle kaldır (hem eski hem grid hücreler)
        document.querySelectorAll('.karno-cell, .karno-cell-grid').forEach(cell => {
            cell.classList.remove('highlight-hover');
    ...
    clearResults() {
        this.groups = [];
        this.isLocked = false; // Sonuçlar temizlendiğinde kilidi kaldır
    ...
    clearAll() {
        ...
        this.karnoValues = [];
        this.isLocked = false; // Kilidi kaldır
    ...
    findGroups() {
        ...
        // this.selectOptimalGroups(activeMinterms); // Devre dışı - findOptimalCovering daha iyi
        console.log('ℹ️ selectOptimalGroups devre dışı - findOptimalCovering kullanılıyor');
        ...
        // Haritayı kilitle
        this.isLocked = true;
        this.renderKarnoMap(); // Kilitli görünümü güncellemek için
        console.log('🔒 Harita kilitlendi - Değişiklik yapmak için "Temizle" butonuna basın');
    ...
    findOptimalCovering(minterms) {
        ...
        for (const group of sortedGroups) {
            ...
            if (newMinterms.length > 0) {
                ...
                console.log(`✅ Grup seçildi: ${group.term} - ${group.size}'lü grup, yeni mintermler: [${newMinterms.join(',')}]`);
    ...
    displayGroups() {
        console.log('📋 displayGroups çağrıldı, this.groups:', this.groups.map(g => g.term));
        const groupsList = document.getElementById('groupsList');
    ...
    showSolution() {
        if (this.groups.length === 0) {
            ...
            if (this.groups.length === 0) {
                ...
                this.isLocked = true; // Boş çözüm için de kilitle
                return;
            }
        }

        this.isLocked = true; // Çözüm gösterildikten sonra kilitle
    ...
    generateSOPForm() {
        console.log('🔧 generateSOPForm çağrıldı, this.groups:', this.groups.map(g => g.term));
        
        if (this.groups.length === 0) {
            return 'F = 0';
        }
        
        // Gruplardan SOP form oluştur
        const terms = this.groups.map(group => group.term).filter(term => term && term !== '1');
    ...
    createLogicGatesSVG() {
        ...
        this.groups.forEach((group, index) => {
            ...
            groupInputs.forEach((inputSignal, inputIndex) => {
                const signalInputY = inputStartY + inputs.indexOf(inputSignal) * inputSpacing;
    ...
    showLockMessage() {
        ...
        message.innerHTML = `
            <div style="text-align: center;">
                🔒 Harita Kilitli!<br>
                <small style="font-weight: normal;">Değişiklik yapmak için önce "Temizle" butonuna basın</small>
            </div>
        `;
